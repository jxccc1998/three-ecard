...
window.addEventListener('click', function(e) {
    ...
    const interspects = raycaster.intersectObject(scene);
    if(interspects.length > 0) {
        if(interspects[0].object.name.includes('hand playerCard') && !finished) {
            hoveredCard = interspects[0].object;
            interspects[0].object.name = hoveredCard.name.replace('hand ', '');
        }
    }
